...
template<typename T>
DenseLayer<T>::DenseLayer(size_t r, size_t c, activations::Activation act)
...
template<typename T>
inline T& DenseLayer<T>::operator()(size_t r, size_t c) {
    return data[r * cols + c];
}

template<typename T>
inline const T& DenseLayer<T>::operator()(size_t r, size_t c) const {
    return data[r * cols + c];
}

template<typename T>
void DenseLayer<T>::randomize() {
    for (auto& x : data) x = random_value<T>();
    for (auto& x : bias) x = random_value<T>();
}

template<typename T>
void DenseLayer<T>::apply_activation(std::vector<T>& v) const {
...
template<typename T>
void DenseLayer<T>::print_memory_usage() const {
...
template<typename T>
void DenseLayer<T>::show_weights() const {
